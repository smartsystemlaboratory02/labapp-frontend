...
import { useDeleteProjectObjectiveMutation } from "~/services/projects/queries";
import ActionModal from "~/components/ui/ActionModal";
import Spinner from "~/components/ui/Spinner";

export const DeleteObjectiveModal = ({
  projectId,
  objectiveId,
}: {
  projectId: string;
  ...
  const {
    ...
    isError,
    error,
  } = useDeleteProjectObjectiveMutation(projectId, objectiveId);

  useEffect(() => {
    if (isError)
      toast.error(
        error.message || "Failed to delete objective. Please try again.",
      );
    if (isSuccess) {
      ...
      {(setOpen) => (
        <div className="flex flex-col gap-6">
          <div className="p-4 rounded-2xl bg-destructive/5 text-destructive text-xs">
            Warning: Deleting this objective will remove it permanently from the
            project. This action cannot be undone. Please confirm that you want
            to proceed with deleting this objective.
          </div>
